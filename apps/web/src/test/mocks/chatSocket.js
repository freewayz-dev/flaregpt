...
export function awaitChatConnection() {
...
export const chatFrame = {
  status: (content) => JSON.stringify({ type: "status", content }),
  token: (content) => JSON.stringify({ type: "token", content }),
  done: (conversationId = null) =>
    JSON.stringify({ type: "done", conversation_id: conversationId }),
  error: (content) => JSON.stringify({ type: "error", content }),
};
