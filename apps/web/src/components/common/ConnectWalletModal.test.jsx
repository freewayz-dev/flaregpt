...
function createHangingConnectorStub(id) {
...
function createResolvingConnectorStub(id) {
  const modalClose = vi.fn();
  const disconnect = vi.fn().mockResolvedValue(undefined);
  const connector = createConnector(() => ({
    id,
...
function createFailingConnectorStub(id, error) {
  const connector = createConnector(() => ({
    id,
    name: id,
    type: "walletConnect",
...
function renderWithHangingWalletConnect(ui) {
...
function renderWithResolvingMetaMask(ui) {
...
function createResumeReconciliationStub(id) {
  const modalClose = vi.fn();
  const disconnect = vi.fn().mockResolvedValue(undefined);
  const connectSpy = vi.fn();
...
function renderWithReconciliationStub(ui) {
...
describe("ConnectWalletModal — MetaMask uses its dedicated connector, other wallets stay on WalletConnect", () => {
  ...
  function renderWithBothConnectors(ui) {
    const wc = createHangingConnectorStub("walletConnect");
...
describe("ConnectWalletModal — MetaMask EIP-6963 detection survives another wallet occupying window.ethereum", () => {
  ...
  function renderWithMetaMaskConnector(ui) {
...
describe("ConnectWalletModal — timeout message matches the connector that actually stalled", () => {
  ...
  function renderWithThreeConnectors(ui) {
    const wc = createHangingConnectorStub("walletConnect");
    const metaMaskSDK = createHangingConnectorStub("metaMaskSDK");
