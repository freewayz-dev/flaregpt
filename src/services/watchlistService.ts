...
export async function fetchWatchlist(signal?: AbortSignal): Promise<WatchlistWallet[]> {
...
export async function updateWatchlistWallet(
  address: string,
  // `nickname: null` (distinct from omitting it) is a real, exercised case
  // — Wallets.jsx sends it to clear a nickname back to the default rather
  // than leaving it unset, and `!== undefined` below deliberately lets
  // `null` through to the request body while an omitted field stays out
  // of it entirely.
  { nickname, newAddress }: { nickname?: string | null; newAddress?: string } = {},
): Promise<WatchlistWallet> {
  const payload: { nickname?: string | null; new_address?: string } = {};
