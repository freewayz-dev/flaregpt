...
export interface WalletActivityEntry {
  // `Hash` (`0x${string}`), not plain `string` — passed directly into
  // getFlarescanTxUrl (web3Config.ts), which requires viem's branded type.
...
interface WalletActivityResponse {
  history: WalletActivityEntry[];
  total_actions_indexed: number;
}
...
export async function fetchWalletActivity(walletAddress: string): Promise<WalletActivityResponse> {
