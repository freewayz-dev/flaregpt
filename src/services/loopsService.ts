...
interface GasSniperStatusResponse {
  opted_in_count: number;
  opted_in_wallets: string[];
  // Entry shape not independently confirmed — see GasSniperCard.test.jsx's
  // own mock, which never needed anything more than an empty array here.
  recent_dry_run_events: unknown[];
}
...
export async function fetchGasSniperStatus(
  signal?: AbortSignal,
): Promise<GasSniperStatusResponse> {
...
export async function enableGasSniper(walletAddress: string): Promise<{ status: string }> {
...
export async function disableGasSniper(walletAddress: string): Promise<{ status: string }> {
