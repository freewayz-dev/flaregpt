...
export async function requestNonce(address: Address): Promise<NonceResponse> {
  const { data } = await flareApi.post("/api/v1/auth/nonce", { address });
  return data;
}

export async function verifySignature(address: Address, signature: Hex): Promise<VerifyResponse> {
