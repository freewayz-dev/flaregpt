...
export async function fetchWalletBalances(walletAddress, signal) {
...
export async function fetchFtsoPortfolio(walletAddress, signal) {
  const { data } = await flareApi.get(
...
export async function fetchFlrPriceHistory(days = 7, signal) {
