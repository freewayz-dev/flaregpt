...
import {
  ...
  getFipProposalUrl,
  ProposalStateValue,
  type ProposalInfoTuple,
  type ProposalVotesTuple,
} from "@/pages/Governance/utils/deriveGovernance";

const PROPOSER = "0x1234567890123456789012345678901234567890" as const;

function makeInfo(overrides: Partial<{
  proposer: string;
  accept: boolean;
  votePowerBlock: bigint;
  voteStartTime: bigint;
  voteEndTime: bigint;
  execStartTime: bigint;
  execEndTime: bigint;
  thresholdBips: bigint;
  majorityBips: bigint;
  circulatingSupply: bigint;
  description: string;
}> = {}): ProposalInfoTuple {
  return [
    (overrides.proposer ?? PROPOSER) as `0x${string}`,
...
describe("buildProposal", () => {
  it("derives a title from the on-chain description's first line, trimmed", () => {
    const proposal = buildProposal(
      ...
      [5_584_276_388_171_569_457_423_523_902n, 110_535_155_356_761_807_469_936_046n] as ProposalVotesTuple,
...
describe("proposal classification", () => {
  const active = buildProposal(1n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Active, "flare", false);
  const pending = buildProposal(2n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Pending, "flare", false);
  const defeated = buildProposal(3n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Defeated, "flare", false);
  const succeeded = buildProposal(4n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Succeeded, "flare", false);
  const queued = buildProposal(5n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Queued, "flare", false);
  const expired = buildProposal(6n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Expired, "flare", false);
  const executed = buildProposal(7n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Executed, "flare", false);
  const canceled = buildProposal(8n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Canceled, "flare", false);
...
describe("computeVoteSplit", () => {
  it("computes real for/against percentages from real vote power", () => {
    const proposal = buildProposal(
      1n,
      makeInfo(),
      [5_584_276_388n, 110_535_155n] as unknown as ProposalVotesTuple,
      ProposalStateValue.Queued, "flare", false
    );
  ...
  it("returns all zeros for a proposal with no votes cast, without dividing by zero", () => {
    const proposal = buildProposal(1n, makeInfo(), [0n, 0n] as ProposalVotesTuple, ProposalStateValue.Pending, "flare", false);
    const split = computeVoteSplit(proposal);
...
describe("computeGovernanceStats", () => {
  it("counts real proposals by real state — matches the 6 live FIP-11..16 proposals' actual distribution", () => {
    const proposals = [
      buildProposal(1n, makeInfo(), [1n, 0n] as ProposalVotesTuple, ProposalStateValue.Queued, "flare", false),
      buildProposal(2n, makeInfo(), [1n, 0n] as ProposalVotesTuple, ProposalStateValue.Queued, "flare", false),
      buildProposal(3n, makeInfo(), [1n, 0n] as ProposalVotesTuple, ProposalStateValue.Active, "flare", false),
      buildProposal(4n, makeInfo(), [1n, 0n] as ProposalVotesTuple, ProposalStateValue.Defeated, "flare", false),
    ];
...
describe("computeUserParticipation", () => {
  it("only counts concluded proposals — voting isn't measurable on something still pending", () => {
    const concluded = [
      buildProposal(1n, makeInfo(), [1n, 0n] as ProposalVotesTuple, ProposalStateValue.Queued, "flare", false),
      buildProposal(2n, makeInfo(), [1n, 0n] as ProposalVotesTuple, ProposalStateValue.Defeated, "flare", false),
    ];
