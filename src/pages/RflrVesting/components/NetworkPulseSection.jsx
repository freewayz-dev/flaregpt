        ...
        <>
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl bg-surface-inset p-4">
              ...
              <p className="text-lg font-semibold text-ink-primary">
                {formatDate(statusQuery.data.next_distribution_date)}
              </p>
              <p className="mt-0.5 text-xs font-medium text-brand">
                {formatCountdown(statusQuery.data.seconds_until_next_epoch, t)}
              </p>
              <p className="mt-2 text-[11px] text-ink-muted">
                {t("rflrVesting.network.epochCaption", {
                  epoch: statusQuery.data.current_epoch_index,
                  date: formatDate(statusQuery.data.epoch_start),
                })}
              </p>
            </div>
            ...
            <div className="rounded-xl bg-surface-inset p-4">
              ...
              <PoolOwnershipBar
                ...
                percentage={parseFloat(emissionsQuery.data.utilization_rate)}
                valueLabel={emissionsQuery.data.utilization_rate}
              />
              <p className="mt-2.5 text-[11px] text-ink-muted">
                {t("rflrVesting.network.remaining", {
                  amount: formatFlrCompact(
                    emissionsQuery.data.incentive_pool_status.remaining_to_be_assigned,
                  ),
                })}
              </p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <MetricTile
              ...
              value={formatFlrCompact(emissionsQuery.data.user_velocity.total_claimed_to_vesting)}
            />
            <MetricTile
              label={t("rflrVesting.network.withdrawnLiquid")}
              value={formatFlrCompact(emissionsQuery.data.user_velocity.total_withdrawn_liquid)}
            />
            <MetricTile
              label={t("rflrVesting.network.currentlyLocked")}
              value={formatFlrCompact(emissionsQuery.data.user_velocity.currently_locked_in_vesting)}
            />
          </div>
        </>
      )}
