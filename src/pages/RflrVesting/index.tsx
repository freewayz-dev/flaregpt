...
export default function RflrVesting() {
  ...
  // The JSX below only ever reads `summary` once `exitQuoteQuery.isSuccess`
  // is true (see the `!isSuccess` branch's own comment) — which is exactly
  // when `summary` is guaranteed non-null, since it's derived from the same
  // query's `data`. TS can't trace that through a separately-computed
  // `useMemo`, so this narrows it back for the branches that actually use
  // it; unused (and therefore never unsafely dereferenced) otherwise.
  const vestingSummary = summary!;
  ...
  return (
    <div className="space-y-5 sm:space-y-6 pb-14">
      ...
      {!activeAddress ? (
        ...
        <>
          <VestingOverviewStats summary={vestingSummary} />
