...
export function computeUnlockTimeline(meltSchedule) {
  ...
  const points = Array.from(byDate.entries())
    .map(([date, amount]) => ({ date, timestamp: new Date(date).getTime(), amount }))
...
export function buildMeltScheduleRows(meltSchedule, t) {
  const entries = meltSchedule?.data ?? [];
  return [...entries]
