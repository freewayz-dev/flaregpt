...
import type { FtsoPortfolioResponse } from "@/services/dashboardService";
...
export interface RewardsSummary {
  ...
  dailyEarning: number;
  targetEpochYield: number;
  accrualRatePerSecond: number;
  calculationMethod: string | null;
  velocitySourceEpoch: number | null;
  isInactive: boolean;
...
export function computeRewardsSummary(portfolio: FtsoPortfolioResponse): RewardsSummary {
...
export function computeDelegationRows(portfolio: FtsoPortfolioResponse): DelegationRow[] {
...
export function computeEpochLedgerChart(portfolio: FtsoPortfolioResponse): { points: EpochPoint[] } {
...
export function buildEpochLedgerRows(
  portfolio: FtsoPortfolioResponse,
): { epoch: number; unclaimed_flr: number }[] {
  const entries = portfolio.unclaimed_epochs_ledger ?? [];
  return [...entries]
