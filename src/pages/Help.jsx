...
const GUIDE_LINKS = NAV_LINKS.filter(
  (link) => Boolean(link.guideDescriptionKey),
);
...
function XLogo({ className, ...props }) {
  ...
  // `className` (e.g. Help's `"h-4 w-4"`) silently overwrite it instead of
  // combining, so this rendered with the SVG's default black fill instead
  // of `currentColor`. Invisible in light mode (black reads close enough to
  // the intended dark gray on a white card), but genuinely broken in dark
  // mode — solid black against a near-black card.
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
