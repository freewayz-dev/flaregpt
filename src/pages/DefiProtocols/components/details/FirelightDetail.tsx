...
import type { FirelightVaultResponse } from "@/services/defiProtocolsService";
...
export default function FirelightDetail({
  ...
  activeAddress,
  icon: Icon,
}: DetailComponentProps<FirelightVaultResponse>) {
  ...
  // `hasData` above already proves `global_analytics`/`user_portfolio` are
  ...
  const vault = data!;
  const backingMultiplier = vault.global_analytics.asset_backing_multiplier ?? 1;
  const balance = vault.user_portfolio.stxrp_balance ?? 0;
  const redeemable = vault.user_portfolio.redeemable_fxrp_value ?? 0;
  const ownershipPct = parsePercent(vault.user_portfolio.pool_ownership);
      ...
      <div className="min-w-0 max-w-xl">
        ...
        <div className="mt-4 divide-y divide-divider">
          <TokenRow
            ...
            value={<SensitiveValue>{`${formatAmount(balance)} ${vault.receipt_token}`}</SensitiveValue>}
            highlight={balance > 0}
          />
          <TokenRow
            symbol="FXRP"
        ...
        <div className="mt-5">
          <PoolOwnershipBar
            ...
            valueLabel={vault.user_portfolio.pool_ownership}
          />
        </div>
      </div>
        ...
        >
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-1">
            <MetricTile
              ...
              value={formatAmount(vault.global_analytics.circulating_supply_stxrp, 0)}
              caption={vault.receipt_token}
            />
            <MetricTile
              icon={(props) => <TokenIcon symbol="FXRP" size={14} {...props} />}
              label={t("defiProtocols.firelight.underlyingReserve")}
              value={formatAmount(vault.global_analytics.underlying_reserve_fxrp, 0)}
              caption="fXRP"
            />
          </div>
          <AddressPill
            label={t("defiProtocols.common.proxyContract")}
            address={vault.proxy_contract}
          />
        </Disclosure>
      </div>
    </div>
