...
export default function SceptreDetail({
  ...
  const exchangeRate = data.global_kpis.sflr_to_flr_exchange_rate ?? 1;
  const balance = data.user_position.sflr_shares_balance ?? 0;
  const redeemable = data.user_position.redeemable_value_flr ?? 0;
  const ownershipPct = parsePercent(data.user_position.pool_ownership);
      ...
      <div className="min-w-0 max-w-xl">
        ...
        <div className="mt-5">
          <PoolOwnershipBar
            ...
            valueLabel={data.user_position.pool_ownership}
          />
        </div>
      </div>
        ...
        >
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-1">
            <MetricTile
              ...
              value={formatAmount(data.global_kpis.total_minted_shares, 0)}
            />
            <MetricTile
              icon={(props) => <TokenIcon symbol="FLR" size={14} {...props} />}
              label={t("defiProtocols.sceptre.backedFlr")}
              value={formatAmount(data.global_kpis.underlying_backed_flr, 0)}
            />
          </div>
          <AddressPill
