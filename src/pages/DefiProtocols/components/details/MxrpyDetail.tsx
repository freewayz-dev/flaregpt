...
import type { MxrpyVaultResponse } from "@/services/defiProtocolsService";
...
export default function MxrpyDetail({
  ...
  activeAddress,
  icon: Icon,
}: DetailComponentProps<MxrpyVaultResponse>) {
  ...
  // `hasData` above already proves `global_metrics`/`user_portfolio` are
  ...
  const vault = data!;
  const shares = vault.user_portfolio.receipt_shares ?? 0;
      ...
      <div className="min-w-0 max-w-xl">
        ...
        <p className="mt-4 text-xs text-ink-muted">
          {t("defiProtocols.mxrpy.circulatingCaption", {
            shares: formatAmount(vault.global_metrics.total_circulating_shares, 0),
            decimals: vault.global_metrics.token_decimals,
          })}
        </p>
      </div>
