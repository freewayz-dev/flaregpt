...
import type { Strategy } from "@/services/defiProtocolsService";

const DEFAULT_AMOUNT = 10000;

function parseAprPercent(value: string) {
  const n = Number.parseFloat(value);
  return Number.isFinite(n) ? n : 0;
}

function formatFlr(value: number) {
  return Number(value).toLocaleString(undefined, { maximumFractionDigits: 2 });
}

function formatLockup(seconds: number, t: TFunction) {
...
function parseTicker(displayName: string) {
...
function VerdictCallout({ strategy, t }: VerdictCalloutProps) {
  return (
...
export default function StrategyComparisonTable() {
  ...
  const [expandedKey, setExpandedKey] = useState<string | false | null>(null);
