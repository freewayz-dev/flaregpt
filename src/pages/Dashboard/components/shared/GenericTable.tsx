...
function formatCell(value: unknown, yesLabel: string, noLabel: string) {
...
interface GenericTableProps {
  ...
  // Optional: FlrPriceChart's own usage never passes these (or `emptyIcon`)
  // because it only ever renders this table when `items` is already
  // confirmed non-empty (guarded by `tableRows.length > 0` at its own call
  // site) — the two Dashboard ledger cards do always pass them, since their
  // items genuinely can be empty. Falls back to an empty string in the
  // (for FlrPriceChart, unreachable) case where `items` is empty and no
  // copy was given.
  emptyTitle?: string;
