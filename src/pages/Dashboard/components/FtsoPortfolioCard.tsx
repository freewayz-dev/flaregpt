...
export default function FtsoPortfolioCard() {
  ...
  // `hasData` above already proves these three fields are present — TS
  // can't trace that through a separately-computed boolean, so this is the
  // one point that narrows `data` back to non-null for the JSX below.
        ...
        <>
          <div className="divide-y divide-divider">
            <TokenRow
              ...
              value={<SensitiveValue>{formatAmount(portfolio.ftso_infrastructure.user_wflr_balance)}</SensitiveValue>}
            />
            <TokenRow
              symbol="FLR"
            ...
            <TokenRow
              ...
              value={portfolio.active_delegations.length}
            />
            <TokenRow
              symbol="FLR"
              label={t("dashboard.ftso.estHourlyEarning")}
              value={<SensitiveValue>{`${formatAmount(portfolio.realtime_estimation.estimated_hourly_earning)} FLR`}</SensitiveValue>}
            />
          </div>
