...
import { useUIStore, type Timeframe } from "@/store/useUIStore";
...
import type { PricePoint, OhlcPoint } from "@/services/dashboardService";

const RANGES: { label: Timeframe; days: number }[] = [
  { label: "1D", days: 1 },
  { label: "7D", days: 7 },
  { label: "30D", days: 30 },
  { label: "1Y", days: 365 },
...
function formatTick(timestamp: number, days: number) {
...
function pickTicks(data: (PricePoint | OhlcPoint)[] | undefined, count = 6) {
...
// Recharts' own types for a custom Bar `shape` render prop don't export a
// usable named type for these fields — this is the minimal real shape this
// component actually reads off it (pixel geometry + the OHLC payload for
// this bar), not the library's full internal prop set.
interface CandlestickShapeProps {
  x: number;
  y: number;
  width: number;
  height: number;
  // Optional to match recharts' real `BarShapeProps` (some internal render
  // passes omit it) — the `!payload` guard below is what actually keeps
  // this component's own OHLC destructuring safe, not the type alone.
  payload?: OhlcPoint;
}
...
function CandlestickShape(props: CandlestickShapeProps) {
  ...
  const pixelForValue = (value: number) => y + ((high - value) / range) * height;
...
function CandlestickTooltip({ active, payload, formatPrice }: CandlestickTooltipProps) {
...
export default function FlrPriceChart() {
  ...
  const { isSuccess, isError, isFetching, refetch } = isCandlestick ? ohlcQuery : priceHistoryQuery;
  // Kept as two separately-typed variables rather than merged into one
  // `data` the way the query's other (shape-independent) fields above are —
  // OhlcPoint and PricePoint have different fields, and recharts' own
  // `data` prop is typed against a single concrete point type per chart, so
  // a merged `(PricePoint | OhlcPoint)[]` doesn't actually satisfy either
  // chart or either field access below. Each branch already knows which
  // one it means; this just lets TypeScript know it too.
  const ohlcData = ohlcQuery.data;
  const priceData = priceHistoryQuery.data;
  const activeLength = isCandlestick ? ohlcData?.length : priceData?.length;

  const formatPrice = (value: number | null | undefined) =>
    formatCurrency(value, { minimumFractionDigits: 4, maximumFractionDigits: 4 });
  const formatTickPrice = (value: number | null | undefined) =>
    value == null ? "" : formatCurrency(value, { minimumFractionDigits: 3, maximumFractionDigits: 3 });

  // recharts' own Tooltip `formatter`/`labelFormatter` prop types are
  // necessarily generic (a chart's dataKey/label can be any value type, not
  // just this one's numeric price/timestamp), so `value`/`label` arrive as
  // the library's own wide `ValueType`/`ReactNode` unions rather than
  // `number` — narrowed here since this chart's own `dataKey="price"` and
  // XAxis `dataKey="timestamp"` are always numeric in practice.
  ...
  const first = isCandlestick ? ohlcData?.[0]?.open : priceData?.[0]?.price;
      ...
      <div className="h-56 sm:h-64 mt-4 min-w-0">
        {isError || !activeLength ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={ohlcData} margin={{ left: 0, right: 8, top: 5, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#94A3B8" opacity={0.15} />
              <XAxis
                dataKey="timestamp"
                type="number"
                domain={["dataMin", "dataMax"]}
                ticks={pickTicks(ohlcData)}
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === "Line" ? (
              <LineChart data={priceData} margin={{ left: 0, right: 8, top: 5, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#94A3B8" opacity={0.15} />
                <XAxis
                  dataKey="timestamp"
                  type="number"
                  domain={["dataMin", "dataMax"]}
                  ticks={pickTicks(priceData)}
                  tickFormatter={(ts) => formatTick(ts, days)}
                  tick={{ fontSize: 10, fill: "#94A3B8" }}
                  axisLine={false}
                  tickLine={false}
              ...
              <AreaChart data={priceData} margin={{ left: 0, right: 8, top: 5, bottom: 0 }}>
                <defs>
