...
export type GovernanceNetwork = "flare" | "songbird";
...
export const CURRENT_POLLING: Record<GovernanceNetwork, { chainId: number; address: Address }> = {
...
export interface HistoricalProposalRef {
  network: GovernanceNetwork;
  chainId: number;
  contractAddress: Address;
  id: bigint;
  // The exact block its ProposalCreated event was emitted in — used to
  // fetch that event (and therefore the real on-chain description) with a
  // single-block eth_getLogs call, since Flare's public RPCs cap
  // eth_getLogs at a 30-block range per call, far too narrow to search an
  // unknown range but trivial once the block is already known.
  blockNumber: bigint;
}
...
export const HISTORICAL_PROPOSALS: HistoricalProposalRef[] = [
