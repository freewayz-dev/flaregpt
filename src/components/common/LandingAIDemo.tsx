...
import type { TextBlock, MessageStatus } from "@/store/useFlareGptStore";
import FlareGptMark from "@/components/common/FlareGptMark";

// Reuses `TextBlock`/`MessageStatus` from the real store — same concept,
// same shape. Deliberately NOT the store's own `ChatMessage` union, though:
// this demo's messages have no `createdAt` (nothing here ever sorts or
// displays one) and a user message has no `status` field at all — forcing
// those in just to match the store's type would be fabricating data this
// component never needs, not genuine reuse.
...
interface DemoAssistantMessage {
  id: string;
  role: "assistant";
...
type DemoMessage = DemoUserMessage | DemoAssistantMessage;
...
function blocksToText(blocks: TextBlock[] | undefined) {
...
export default function LandingAIDemo({ onOpenWalletModal }: LandingAIDemoProps) {
  const { hasSession, isConnected, isAuthenticating, signIn } = useAuthStatus();
  const [messages, setMessages] = useState<DemoMessage[]>([]);
  const [isGenerating, setIsGenerating] = useState(false);
  const [value, setValue] = useState("");
  const streamRef = useRef<ReturnType<typeof streamChatMessage> | null>(null);
  ...
  const handleSend = (text: string) => {
    ...
    const userMessage: DemoUserMessage = { id: makeId(), role: "user", content: trimmed };
    const assistantMessage: DemoAssistantMessage = {
      id: makeId(),
      role: "assistant",
      status: "thinking",
      blocks: [],
    ...
    const updateAssistant = (patch: Partial<DemoAssistantMessage>) =>
      setMessages((prev) =>
        prev.map((m) =>
          m.id === assistantMessage.id && m.role === "assistant" ? { ...m, ...patch } : m,
        ),
      );
