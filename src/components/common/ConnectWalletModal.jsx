...
export default function ConnectWalletModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center sm:p-4">
      <div
        onClick={onClose}
        className={`absolute inset-0 bg-black/40 backdrop-blur-[4px] transition-opacity duration-200 ease-out ${
          animate ? "opacity-100" : "opacity-0"
        }`}
      />

      <div
        className={`relative w-full bg-[#FFFFFF] p-5 shadow-xl transition-all duration-200 ease-out dark:bg-[#161619] rounded-t-2xl max-w-none transform-gpu sm:relative sm:rounded-2xl sm:max-w-md ${transitionStyles}`}
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-[#E5E7EB] dark:border-[#1D1D20]">
          <div>
            ...
            <p className="text-[11px] text-[#475569] dark:text-[#A1A1AA] mt-0.5">
              Select your preferred gateway to the Flare Network.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-5 text-[10px] text-[#94A3B8] dark:text-[#71717A] text-center leading-relaxed">
          Using a mobile browser app? The wallet's internal browser will connect
          automatically via your extension interface.
        </div>
      </div>
    </div>
  );
