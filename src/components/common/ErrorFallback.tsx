...
// `resetErrorBoundary`.
export default function ErrorFallback({ error, resetErrorBoundary, variant = "page" }: ErrorFallbackProps) {
  const isRoot = variant === "root";
  // `error` is `unknown` (react-error-boundary's own FallbackProps type —
  // an ErrorBoundary can catch a thrown non-Error value, so it can't
  // guarantee more than that) rather than the `error?.message` optional
  // chain the original JS got away with.
