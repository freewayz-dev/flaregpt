  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button
        ...
        className="flex w-full items-center gap-2 rounded-xl border border-[#E5E7EB] bg-[#FFFFFF] px-3 py-3 text-sm text-left text-ink-primary dark:bg-[#21242B] dark:border-none focus:outline-none focus:border-brand dark:focus:border-brand transition-colors duration-200 relative z-10 cursor-pointer"
      >
        {selectedValue?.flag && <span className="shrink-0">{selectedValue.flag}</span>}
        <span className="truncate">{renderLabel(selectedValue)}</span>
        <ChevronDownIcon className={`ml-auto h-4 w-4 text-ink-muted transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
      </button>
