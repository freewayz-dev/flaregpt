...
} from "@/components/flareGpt/WalletAddressBadge";
import TokenBadgesInline, {
  type TokenBadgesInlineProps,
} from "@/components/flareGpt/TokenBadgesInline";
import ChartBlock, { type ChartBlockProps } from "@/components/flareGpt/ChartBlock";
import ThinkingIndicator from "@/components/flareGpt/ThinkingIndicator";
import type { AssistantChatMessage, TextBlock } from "@/store/useFlareGptStore";

// The real API only ever produces "text" blocks (see useFlareGptStore.ts's
// `AssistantChatMessage.blocks: TextBlock[]`) — these other three variants
// are dead code paths today, kept because this renderer already handled
// them before the migration and the backend's own capability may grow into
// them later (see the original component's comment). Each type reuses its
// target component's own prop type rather than re-declaring the same
// shape a second time.
type WalletBadgeBlock = { type: "walletBadge" } & WalletAddressBadgeProps;
type TokenBadgesBlock = { type: "tokenBadges" } & TokenBadgesInlineProps;
type ChartMessageBlock = { type: "chart" } & ChartBlockProps;
type RenderableBlock = TextBlock | WalletBadgeBlock | TokenBadgesBlock | ChartMessageBlock;
...
function AssistantMessage({ message, isLast, onRegenerate }: AssistantMessageProps) {
