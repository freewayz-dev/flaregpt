  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      if (mobileMenuRef.current && !(target && mobileMenuRef.current.contains(target))) {
        if (!(target instanceof Element && target.closest(".mobile-trigger-btn"))) {
      ...
      {!hideAskFlareGpt && (
        <>
          {showFabHint && (
            ...
            // 104px = 40px + 52px + 12px; then 116px = 48px + 56px + 12px;
            // now 100px = 32px + 56px + 12px, after moving the FAB itself
            // slightly lower on mobile) — keeps the callout arrow pointing
            // at the same spot just above the button, not floating off
            // from it.
            <div
              role="status"
              onClick={dismissFabHint}
              className="lg:hidden fixed bottom-[calc(100px+env(safe-area-inset-bottom))] right-[calc(1.25rem+env(safe-area-inset-right))] z-30 max-w-[190px] rounded-xl bg-ink-primary px-3 py-2 text-[11px] font-medium leading-snug text-white shadow-xl cursor-pointer"
            >
          ...
          <button
            ...
            className="lg:hidden fixed bottom-[calc(2rem+env(safe-area-inset-bottom))] right-[calc(1.25rem+env(safe-area-inset-right))] z-30 flex items-center justify-center h-14 w-14 rounded-full bg-brand text-white shadow-xl hover:scale-105 active:scale-95 transition-all cursor-pointer border border-white/10"
