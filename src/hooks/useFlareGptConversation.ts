...
} from "@/store/useFlareGptStore";
import { useAuthStore } from "@/store/useAuthStore";
import type { WalletHubEntry } from "@/store/useWalletHubStore";
import * as chatService from "@/services/chatService";
import { streamChatMessage, type ChatHistoryEntry } from "@/services/chatSocket";
...
function toTextBlocks(markdown: string): TextBlock[] {
  return [{ type: "text", markdown }];
}

function blocksToText(blocks: TextBlock[] | undefined): string {
  return (blocks ?? [])
...
// (nonexistent) server id. Written as an if/else (rather than the
// spread-a-shared-base-object shape this started as) so each branch
// produces a value TypeScript can actually check against ChatMessage's
// discriminated union — a shared `base` object typed `role` as the wider
// "user" | "assistant" independent of which branch ran, which doesn't
// narrow cleanly; this has no behavioral difference from the original.
function fromHistoryEntry(entry: {
  role: "user" | "assistant";
  content: string;
  timestamp: number;
}): ChatMessage {
  const createdAt = entry.timestamp ? entry.timestamp * 1000 : Date.now();
  if (entry.role === "user") {
    return { id: makeId(), role: "user", status: "complete", createdAt, content: entry.content };
  }
  return {
    id: makeId(),
    role: "assistant",
    status: "complete",
    createdAt,
    blocks: toTextBlocks(entry.content),
  };
}
...
function withWalletContext(text: string, effectiveWallet: WalletHubEntry | null): string {
...
export function useFlareGptConversation() {
  ...
  useEffect(() => {
    ...
    // `queryClient` (from `useQueryClient()`) is referentially stable for
    // the life of this app — one `QueryClient` instance is created once in
    // main.tsx and never swapped — so including it here doesn't cause any
    // extra effect runs; it's just the honest way to satisfy the rule
    // instead of suppressing it.
  }, [hasSession, activeConversationId, t, queryClient]);
  ...
  interface ActiveRequestState {
    cancelled: boolean;
    streamHandle: ReturnType<typeof streamChatMessage> | null;
  ...
  const requestReply = async (userText: string, guestHistory: ChatHistoryEntry[]) => {
    ...
    const requestState: ActiveRequestState = { cancelled: false, streamHandle: null, reject: null };
    ...
    let justCreatedConversationId: string | null = null;
    ...
    try {
      ...
      let fullText = "";
      await new Promise<{ conversationId: string | null }>((resolve, reject) => {
        requestState.reject = reject;
        requestState.streamHandle = streamChatMessage(withWalletContext(userText, effectiveWallet), {
          // `effectiveWallet.address` is a plain `string` here (see
          // useWalletHubStore.ts — a guest's tracked-wallet address comes
          // from free-typed input, only loosely validated by Wallets.jsx's
          // handleSave, never checked against a real EIP-55 checksum),
          // while streamChatMessage expects viem's branded `Address`.
          // `isAddress` is a real runtime check, not an assertion — a
          // malformed guest-typed address (should Wallets.jsx's own
          // validation ever be loosened) is dropped here rather than sent
          // as if it were verified, matching this call's existing
          // fallback-through-undefined behavior for "nothing usable."
          address:
            effectiveWallet?.address && isAddress(effectiveWallet.address)
              ? effectiveWallet.address
              : undefined,
          conversationId: hasSession ? (conversationId ?? undefined) : undefined,
  ...
  const regenerate = () => {
    ...
    // `findIndex` above already proved this is a user message, but that
    // narrowing doesn't survive the array index above it — this re-checks
    // the same discriminant so `lastUser.content` below is real, not an
    // assertion past what's actually known.
  ...
  const deleteConversation = async (id: string | null = activeConversationId) => {
