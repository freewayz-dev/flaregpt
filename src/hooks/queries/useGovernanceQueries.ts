...
export function useProposalIds(network: GovernanceNetwork) {
...
export function useProposals(network: GovernanceNetwork, ids: readonly bigint[] | undefined) {
  const { chainId, address } = CURRENT_POLLING[network];
  ...
  const proposals = useMemo<Proposal[] | undefined>(() => {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      result.push(
        buildProposal(
          ids[i],
          info.result as ProposalInfoTuple,
          votes.result as ProposalVotesTuple,
          state.result as number,
          network,
          false,
        ),
      );
...
export function useHistoricalProposals(network: GovernanceNetwork) {
    ...
    queryFn: async () => {
      ...
      const entries = await Promise.all(
        refs.map(async (ref) => {
          ...
          return [ref.id.toString(), description] as const;
        }),
      );
...
export function useCurrentVotingPower(network: GovernanceNetwork, activeAddress: Address | undefined) {
  const { chainId } = CURRENT_POLLING[network];
